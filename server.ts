...
import { getEnv, isHubTokenDerivedFromPath } from './src/lib/server/config.ts';
...
server.listen(port, host, () => {
	...
	console.log(`[hub] Recorder PWA:  ${loopback}/recorder  (use HTTPS for mic; see README)`);
	console.log(
		`[hub] WebSocket STT: ${loopback.replace('http', 'ws')}/api/audio/stream  (same-origin upgrade as page)`
	);
	if (lans.length) {
		for (const o of lans) {
			console.log(`[hub] On LAN (device):  ${o}/recorder  |  ${o.replace('http', 'ws')}/api/audio/stream`);
		}
	} else {
		console.log('[hub] (no non-loopback IPv4; set HOST/PORT or use tailscale funnel — see README)');
	}
	const tok = getEnv();
