...
All **per-repo** settings are command-line flags (no `~/.config` or env for hub URL / token / paths).

```bash
pnpm thepm bridge --help
# (legacy: pnpm thepm-bridge --help)
```
...
You still need this **git checkout** and `node_modules` (the CLI runs `tsx` + `src/bridge-cli.ts` from the repo). The process **cwd** is the directory where you run the command (same as global `thepm bridge`), so relative `--project-root` / `--prd` resolve correctly.

1. From the repo root, install deps once: `pnpm install`
2. Link the package globally: `pnpm link --global` (or `npm link`)

If you copy only [`bin/thepm.mjs`](../../bin/thepm.mjs) (or the legacy `bin/thepm-bridge.mjs`) elsewhere, set **`THEPM_ROOT`** to the absolute path of this repository and pass the same flags as above.

**`rg` (ripgrep)** must be on `PATH` on the machine running the bridge.
...
The CLI is `src/bridge-cli.ts` and uses the same `executeCodeOp` as the hub. To produce a single executable without the full repo on disk, use a bundler (e.g. **esbuild** or **pkg**) and document that **`rg` is required** (or ship a [ripgrep](https://github.com/BurntSushi/ripgrep) binary next to the tool).
