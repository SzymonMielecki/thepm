...
const State = Annotation.Root({
  ...
  speakerId: Annotation<string | null>({
    reducer: (_x, y) => y,
    default: () => null,
  }),
  ...
  intent: Annotation<import("./types").IntentOutput | null>({
    reducer: (_x, y) => y,
    default: () => null,
  }),
  rg: Annotation<
...
export function utteranceSuggestsTicketCue(utterance: string): boolean {
  ...
  const cues = [
    /\bimplement(s|ed|ing)?\b/,
    /\bbuild(s|ed|ing)?\b/,
    /\brefactor(s|ed|ing)?\b/,
    /\boptimize(s|ed|ing)?\b/,
    /\bimprove(s|ed|ing)?\b/,
    /\bfix(es|ed|ing)?\b/,
    /\badd(s|ed|ing)?\b/,
    /\bremove(s|ed|ing)?\b/,
...
async function nodePrd(s: S): Promise<Partial<S>> {
  if (!s.intent) return {};
  const rg = s.rg ?? [];
  const pendingPrdPatch = await runPrdPatchFromContext(
    s.utterance,
    s.intent,
    rg,
    s.sessionId,
...
function compile(_database: AppDatabase) {
  ...
  g.addConditionalEdges(
    "classify_intent" as any,
    (st: S) => routeAfterIntent(st),
    {
      noise: "end" as any,
  ...
  g.addConditionalEdges("prd" as any, (_st: S) => "draft", {
    draft: "draft" as any,
  });
