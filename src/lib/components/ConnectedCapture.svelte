  ...
  {#if devices.length === 0}
    <p class="text-xs text-zinc-600">
      Open <a
        ...
        target="_blank"
        rel="noopener noreferrer">mobile capture</a
      > on a device.
    </p>
  {:else}
    <ul class="flex flex-col gap-1.5 sm:flex-row sm:flex-wrap">
      {#each devices as d (d.id)}
        ...
        >
          ...
          <span class="font-mono text-emerald-100/90" title={d.sessionId}
            >{shortSession(d.sessionId)}</span
          >
          <span class="shrink-0 text-zinc-500">·</span>
          <span class="min-w-0 truncate text-zinc-400" title={d.userAgent ?? ""}
            >{shortUa(d.userAgent)}</span
          >
          <span class="shrink-0 text-zinc-600"
            >{formatAddr(d.remoteAddress)}</span
          >
          <span class="shrink-0 text-zinc-600">
